...
function setResumeId(id) {
  if (id) localStorage.setItem("resume_id", id);
}
...
async function initIndex() {
  ...
  if (resumeInput) {
    resumeInput.addEventListener("change", async () => {
      if (!resumeInput.files || !resumeInput.files.length) return;
      const file = resumeInput.files[0];

      if (resumeStatus) resumeStatus.textContent = "Uploading & parsing resume...";
      ...
      try {
        ...
        if (resumeStatus) {
          ...
          resumeStatus.textContent = data.resume_id
            ? `Resume ready. Detected skills: ${skills.join(", ") || "(none)"}`
            : "Resume ready.";
        }
      } catch (e) {
        clearResumeId();
        if (resumeStatus) resumeStatus.textContent = e.message || "Resume upload failed";
      }
    });
  }
...
async function initInterview() {
  ...
  function speakText(text) {
    ...
    try {
      ...
      alert("Unable to speak the question.");
    }
  }

  if (speakBtn) {
    speakBtn.addEventListener("click", () => {
      speakText(qBox ? qBox.textContent : "");
    });
  }
  ...
  function setListening(on) {
    isListening = !!on;
    if (micBtn) micBtn.dataset.listening = isListening ? "1" : "0";
  }
  ...
  function ensureRecognition() {
    ...
    recognition.addEventListener("result", (e) => {
      ...
      if (answerEl && transcript) {
        const prev = answerEl.value ? String(answerEl.value).trim() : "";
        answerEl.value = prev ? (prev + " " + transcript) : transcript;
        answerEl.focus();
      }
    });
  ...
  if (micBtn) {
    micBtn.addEventListener("click", () => {
      ...
      try {
        ...
        setListening(false);
        alert("Microphone permission was denied or speech recognition failed to start.");
      }
    });
  }

  const analyticsBoxId = "ai-analytics";
  let analyticsBox = document.getElementById(analyticsBoxId);
  if (!analyticsBox) {
    analyticsBox = document.createElement("div");
    analyticsBox.id = analyticsBoxId;
    analyticsBox.className = "glass-card";
    analyticsBox.style.marginTop = "12px";
    analyticsBox.innerHTML = "<div style='opacity:0.85'>AI evaluation will appear here after you submit.</div>";
    const panel = document.querySelector(".main-panel");
    if (panel) panel.appendChild(analyticsBox);
  }
  ...
  function setStage(nextStage) {
    stage = nextStage;
    if (submitBtn) submitBtn.textContent = stage === "next" ? "Next Question" : "Submit Answer";
  }

  function setAnalyticsPlaceholder() {
    if (analyticsBox) analyticsBox.innerHTML = "<div style='opacity:0.85'>AI evaluation will appear here after you submit.</div>";
  }
  ...
  if (submitBtn) {
    submitBtn.addEventListener("click", async () => {
      if (stage === "next") {
        if (!pendingNext) {
          pendingNext = await jsonGet(`/get-question?session_id=${encodeURIComponent(sid)}`);
        }

        safeText(qBox, pendingNext.question_text);
        safeText(qIndex, (pendingNext.index || 0) + 1);
        safeText(qTotal, pendingNext.total);

        setProgress(prog, (((pendingNext.index || 0) + 1) / (pendingNext.total || 1)) * 100);
      ...
      try {
        ...
        if (analyticsBox) {
          const label = res.label ? `<span class='pill' style='margin-left:8px'>${res.label}</span>` : "";
          const r = res.rubric || {};
          const rows = [
            ["Technical", r.technical_accuracy],
          ...
          analyticsBox.innerHTML =
            ...
            `<div>Score: <strong>${Number(res.score_10 || 0).toFixed(1)}/10</strong>${label}</div>` +
            `</div>` +
            `<div style='margin-top:8px; opacity:0.9'>${(res.feedback || "").toString()}</div>` +
            (rows.length ? (`<div style='margin-top:10px; display:grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap:8px'>` +
              rows.map(([k,v]) => `<div class='glass-card' style='padding:10px'><div style='opacity:0.8'>${k}</div><div style='font-size:18px; font-weight:800'>${Number(v||0).toFixed(1)}/10</div></div>`).join("") +
            `</div>`) : "");
        }
...
function ensureChatbot() {
  ...
  panel.innerHTML =
    `<div class='chatbot-header'><strong>Assistant</strong><button type='button' id='chatbot-close' class='chatbot-close' aria-label='Close'>×</button></div>` +
    `<div id='chatbot-messages' class='chatbot-messages'></div>` +
    `<form id='chatbot-form' class='chatbot-form'>` +
    `<input id='chatbot-input' class='chatbot-input' placeholder='Ask about features…' />` +
    `<button class='btn-3d' type='submit' style='padding:10px 14px'>Send</button>` +
    `</form>`;
  ...
  fab.addEventListener("click", () => setOpen(panel.dataset.open !== "1"));
  ...
  function addMsg(text, who) {
    ...
    div.className = who === "me" ? "chatbot-msg chatbot-msg--me" : "chatbot-msg";
  ...
  if (form) {
    form.addEventListener("submit", async (e) => {
      ...
      try {
        const res = await jsonPost("/api/chat", { message: msg });
        addMsg((res.reply || "").toString(), "bot");
      } catch (err) {
        addMsg(err.message || "Assistant failed", "bot");
      }
    });
  }
}
