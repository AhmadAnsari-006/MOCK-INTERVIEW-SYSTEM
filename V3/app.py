...
def start():
    ...
    return render_template(
        "interview.html",
        question=picked[0]["question_text"],
        total=len(picked)
    )
...
def result():
    ...
    feedback = _feedback_statement(score_display)
    ...
    history_labels = [str(i + 1) for i in range(len(history_scores))]

    improvement_tips = _creative_improvement_tips(score_display, report)
    ...
    return render_template(
        ...
        performance_history={
            ...
            "previous_attempts": max(0, len(attempts_before)),
