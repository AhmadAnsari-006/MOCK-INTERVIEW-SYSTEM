...
from automation.report_service import build_analytics_report
from ml_models.performance_predictor import PerformancePredictor


def create_interview_blueprint(question_service, session_service, ai_service=None):
    ...
    db_service.init_db()
    predictor = PerformancePredictor(model_path=(question_service._questions_path.parent / "ml" / "performance_model.joblib"))
    predictor.load_if_present()
    ...
    def start_interview():
        ...
        resume_id = request.args.get("resume_id") or session.get("resume_id")
        resume_profile = session.get("resume_profile") if resume_id else None

        picked = []
        if ai_service and resume_profile:
            try:
                picked = ai_service.generate_questions(
        ...
        if ai_service:
            try:
                eval_result = ai_service.evaluate_answer(
                    ...
                    answer_text=answer,
                    resume_id=sess.get("resume_id") or session.get("resume_id"),
                )
                score_10 = float(eval_result.get("overall_score") or 0)
                eval_result = {
                    "score_10": score_10,
                    "matched_keywords": q.get("keywords") or [],
                    "missing_keywords": [],
                    "feedback": eval_result.get("feedback") or "",
                    "label": eval_result.get("label"),
                    "rubric": eval_result.get("scores"),
    ...
    def get_feedback():
        ...
        analytics = build_analytics_report(per_question=per_question)
        ...
        # ML prediction + skill-gap analysis (lightweight; trains only when enough history exists)
        skill_gaps = (analytics.get("weaknesses") or [])[:10]
        try:
            # Train if we have enough history from DB
            if user_id:
                # synthesize attempts list from the DB score series (minimal dataset)
                # if you later add a full attempt list fetch, plug it in here.
                history_attempts = []
                for s in (db_history_scores or [])[-30:]:
                    history_attempts.append({**latest_attempt_for_ml, "average_score_10": float(s)})
                if history_attempts:
                    predictor.fit(attempts=history_attempts)
        except Exception:
            pass

        try:
            pred = predictor.predict(latest_attempt=latest_attempt_for_ml, skill_gaps=skill_gaps)
            ml_out = {
                "predicted_score_10": pred.predicted_score_10,
                "confidence_0_1": pred.confidence_0_1,
                "skill_gaps": pred.skill_gaps,
            }
        except Exception:
            ml_out = None
